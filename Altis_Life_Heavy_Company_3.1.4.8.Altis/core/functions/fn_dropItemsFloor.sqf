...
if (_death) then {
	...
	_data2 = [_data2];
	if(life_is_processing) exitWith {hint "Vous etes en train de process petit malin.";_exit = true;};
	if(player != vehicle player) exitWith {titleText["Vous ne pouvez pas jeter un objet lorsque vous etes dans un vehicule.","PLAIN"];_exit = true;};
	if(side player != west) then {
		_ind = [_data,life_illegal_items] call TON_fnc_index;
		if(_ind != -1 && ([west,getPos player,100] call life_fnc_nearUnits)) exitWith {titleText["C'est un objet illegal et les flics sont a proximite, vous ne pouvez jetez pas cette preuve","PLAIN"];_exit = true;};
	};
};
if(_exit) exitWith {};
...

{
	...
	if (_death) then {
		...
		if(!([false,_var,(parseNumber _value)] call life_fnc_handleInv)) exitWith {hint "Vous ne pouvez pas lacher autant d'objet, peut-etre que vous n'en avez pas assez?";_exit = true;};
