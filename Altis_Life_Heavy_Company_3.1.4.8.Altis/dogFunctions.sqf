...
_actions = 
	...
	while {(_unit getvariable "action")=="true"} do
		...
		if ((_unit getvariable "order") == "nil") then
			{
				_whistle = _unit addAction ["Достать собаку", _dogWhistle, [_unit, _dogGrowl,_playSound,_dogReturnIdle,_dogVehicle]];
		...
		if ((_unit getvariable "order") == "idle") then
			{
				_follow = _unit addAction ["<t color = '#ffff00'>Следуй</t>", _dogFollow, [_unit,_playSound]];
				_find = _unit addAction ["<t color = '#ffff00'>Искать</t>", _dogSeek, [_unit,_playSound]];
				_rest = _unit addAction ["<t color = '#ff0000'>Спрятать собаку!</t>", _dogHide, [_unit,_playSound]];
				_heel = _unit addAction ["<t color = '#ffff00'>Сидеть</t>", _dogHeel, [_unit,_playSound]];
				_unit setvariable ["step","wait"];
		...
		if ((_unit getvariable "order") == "active") then
			{
				_stop = _unit addAction ["<t color = '#ff0000'>Стоп!</t>", _dogStop, [_unit,_playSound]];
				_unit setvariable ["step","wait"];
